 ...
 */
public class OpenCimiRmModel implements RmModel {
    private static OpenCimiRmModel instance;

    private final RmTypeGraph rmTypeGraph;

    /**
     * Use OpenCimiRmModel.getInstance() instead
     */
    private OpenCimiRmModel() {
        rmTypeGraph = new RmTypeGraphBuilder(org.opencimi.jaxb.rm.ObjectFactory.class).build();
    }

    public static OpenCimiRmModel getInstance() {
        OpenCimiRmModel result = instance;
        if (result == null) {
            result = new OpenCimiRmModel();
            instance = result;
        }
        return result;
    }
